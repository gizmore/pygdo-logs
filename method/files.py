...
class files(UserMethod):
    def gdo_execute(self):
        ...
        for item in LogFiles.from_module(self._module).list(target):
            ...
            rows.append(f'<tr><td><a href="{url}">{html.escape(item.name)}</a></td><td>{Files.human_file_size(item.size)}</td><td>{datetime.fromtimestamp(item.mtime):%Y-%m-%d %H:%M} ({Time.display_age_ts(item.mtime)})</td></tr>')
